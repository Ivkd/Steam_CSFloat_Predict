import { getFullStats, getReferences, getCatalog, getMarket, getTraders, getMLStats, trainModel, getPriceCoverage, } from "./API/apiService.js";
const navItems = [
...
function renderNav(items) {
    ...
    items.forEach((item) => {
        ...
        if (currentPath.includes(`${item.id}.html`) ||
...
async function loadDashboard() {
    ...
    if (prices) {
        ...
        set("price-st-updated", prices.steam.last_updated
...
async function loadDockerPage() {
    ...
    if (!result || result.error) {
        container.innerHTML = `<div class="docker-error">⚠️ Docker недоступен: ${result?.error ?? "нет ответа"}</div>`;
        return;
    }
    ...
    container.innerHTML = services.map(s => {
        ...
        const ports = (s.Publishers ?? []).map((p) => `${p.PublishedPort}→${p.TargetPort}`).join(", ") || "—";
        return `
...
window.showDockerLogs = async (service) => {
    const box = document.getElementById("docker-logs");
    if (box)
        box.textContent = "Загрузка логов...";
    const result = await window.desktopAPI.dockerLogs(service);
    if (!box)
        return;
    if (!result || result.error) {
        box.textContent = `Ошибка: ${result?.error ?? "нет ответа"}`;
        return;
    }
    box.textContent = result.stdout || "Логи пустые";
};
window.dockerAction = async (action, service) => {
    const btn = document.querySelector(`[data-service="${service}"] button`);
    if (btn) {
        btn.disabled = true;
        btn.textContent = "...";
    }
...
window.dockerActionAll = async (action) => {
    const allBtns = document.querySelectorAll(".docker-btn");
...
async function loadMLPage() {
    ...
    const trainBtn = document.getElementById("ml2-train-btn");
...
async function handleTrainClick() {
    const btn = document.getElementById("ml2-train-btn");
    ...
    if (log) {
        log.textContent = "Запускаем обучение модели...";
        log.style.color = "#94a3b8";
    }
    const result = await trainModel();
    if (btn) {
        btn.disabled = false;
        btn.textContent = "🚀 Запустить обучение";
    }
    if (!result) {
        if (log) {
            log.textContent = "❌ Ошибка: API недоступно";
    ...
    if (!result.success) {
        if (log) {
            log.textContent = `❌ Ошибка: ${result.error}`;
            log.style.color = "#ef4444";
        }
        return;
    }
