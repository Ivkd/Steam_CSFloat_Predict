...
ipcMain.handle("docker-action", async (_event, { action, service }) => {
  ...
  if (!allowed.includes(action)) return { error: "Недопустимое действие" };
  if (!service) return { error: "Не указан контейнер" };
...
ipcMain.handle("docker-logs", async (_event, container: string) => {
  try {
    if (!container) return { error: "Не указан контейнер" };
